...
| 项 | 决策 |
|----|------|
| 触发 | 用户要求"先把合规相关功能暂时屏蔽，包括签名功能" |
| 范围 | **A. 仅电子签名**（不含 DHF / IEC 62304 / 审计日志）|
| 实施方式 | **方案 1：Feature Flag 配置** |
| 屏蔽环境 | **全部**（dev / staging / production）|
| 数据处理 | **保留**（不删除历史签名记录）|
| 前端提示 | 是（Dashboard 顶部 banner — 可选）|
...
**⚠️ 重要提醒**：生产环境合规功能（签名）**必须**在合适时机恢复。R220 是**临时**措施，不是永久方案。
