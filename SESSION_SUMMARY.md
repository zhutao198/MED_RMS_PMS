...
## 🛑 Phase 21: R255 电子签名永久下线（2026-07-29）

### 触发
用户原话："**合规相关签名先走线下流程，系统先不考虑电子签名，把这一点记入项目日志，除非我明确恢复电子签名功能，否则一概不考虑**"

### 决策性质
**永久架构决策**（非临时 Feature Flag）——R220（v1.76 临时屏蔽）起升级为 R255 永久决策

### 系统边界（按 R255）
| 维度 | 状态 |
|------|------|
| 电子签名意图/执行/验证/密码设置/OTP | **不在系统范围** |
| 签名相关读端点（历史查询） | 保留 |
| 21 CFR Part 11 §11.50/§11.70/§11.200 合规 | **业务团队线下流程**（纸质签字 + 扫描存档） |
| 后续代码变更 | **禁止新增/恢复**签名相关代码路径（除非用户明确授权） |

### 影响代码
- `med-rms-change/ChangeService.java`：`executeChange`/`approveChange` 中 `signatureId == null` 校验暂时保留（R220 屏蔽期一致行为），后续按 R256+ 计划移除
- `med-rms-esignature/`：写端点继续屏蔽，读端点保留
- `ChangeList.vue` 第 92 行 / `ChangeExecute.vue`：执行流程整合，按 R256+ 计划重构

### 待办（R256+ 候选）
- 移除后端 signatureId 硬性校验（让 R255 决策在代码层彻底落地）
- 重构 ChangeList.vue"执行"按钮：跳 ChangeExecute.vue（设计意图）而非 ElMessageBox 直调后端
- 修复 ChangeList.vue 第 415 行 `catch {}` 静默吞错（让错误可见）
- ChangeExecute.vue 移除 ESignPopup 集成

### 经验教训
1. **决策固化是合规架构的关键动作**：用户临时 Feature Flag（屏蔽）→ 正式决策（永久下线）的转化，必须显式创建 R 节点 + 更新 3 类核心文件 + 跨文档同步，否则后续 AI 会误判"可恢复"
2. **"明确恢复"作为硬约束**：R255 决策原文"除非我明确恢复...否则一概不考虑"是后续 R 节点的反向护栏——遇到任何恢复签名的代码修改都应停下询问
3. **变更管理边界的最终答案**：CR = 需求变更（含级联测试用例）；签名 = 线下流程；两者解耦后 UX 和合规可分别优化
...
## 🚀 Phase 23: R257 变更管理创建入口优化（2026-07-29）

### 触发
用户决策：
1. 变更对象必须是基线化需求（已实现，前后端双重防御）
2. 新建变更需先选项目再选需求

### 边界强化
| 子项 | 改动 | 关键文件 |
|------|------|---------|
| **R257.1** | ChangeCreate.vue 加 ProjectSelector 顶部一级 + 二级联动需求（仅该项目 Baseline 需求） | `ChangeCreate.vue` |
| **R257.2** | ChangeList.vue 移除内嵌创建对话框（输入 ID 反模式），按钮跳 `/changes/create` | `ChangeList.vue` |
| **R257.3** | 接入 R192 全局项目选择同步（进入页面采用 store.currentProjectId + syncToStore=true） | `ChangeCreate.vue` |

### UX 改进
- **消除跨项目误选**：多项目用户不会再看到其他项目的已基线需求
- **明确边界提示**：空状态显示"该项目暂无已基线需求"+"未基线化需求可直接编辑修改"
- **双入口收敛**：列表页和创建页不再重复实现创建逻辑
- **与 R192 一致**：所有变更管理页面都遵循全局项目选择同步约定

### 验证
- 前端：`vue-tsc --noEmit` → **0 error**
- 残留引用：grep `showCreateDialog|createForm|createChange` → **0 match**
- 后端：未改动（Baseline 校验仍生效）
...
### 经验教训
1. **输入数字 ID 是反模式**：R257.2 移除 ChangeList 的输入 ID 框，证明"表单字段应该是有意义的 UI 控件"原则
2. **R192 全局项目同步的复用价值**：R257.3 直接用现成的 ProjectSelector + useProjectStore，0 新代码
3. **空状态的设计价值**：el-empty 配以业务提示文案，比纯空 list 更友好——用户立刻明白"为什么没数据"
4. **双重防御仍然重要**：前端友好引导（仅显示 Baseline）+ 后端硬性校验（拒绝非 Baseline），是合规系统的标准模式
5. **路由跳转 vs 内嵌对话框**：删除"内嵌创建对话框"是个明确的架构决策——避免双入口状态不一致


---

## 🏁 Phase 8: 收尾汇总（2026-07-30）

### R255-R267 共 13 个 R 节点产出

| 节点 | 关键改动 |
|------|---------|
| **R255** | 电子签名走线下流程（永久架构决策）|
| **R256** | R255 决策代码落地（移除后端签名校验 + ChangeList 跳转 ChangeExecute）|
| **R257** | 变更管理创建入口优化（项目→需求二级联动 + 移除输入 ID 反模式）|
| **R259** | 修复 ProjectService.create SY0000（嵌套聚合 + 非数字编号双 bug）|
| **R258.1-R258.2** | URS + PRS 草稿创建测试 |
| **R260** | 移除电子签名 UI 残留（5 个业务页面）|
| **R258.3-R258.6** | SRS/DRS 草稿 + 编辑 + 基线化 + 变更边界验证 |
| **R261** | 修复变更类型枚举对齐 + 基线锁定状态联动 |
| **R262** | CR-102-0001 端到端流程验证（SUBMIT→CLOSED 6 步）|
| **R263** | 修复 performImpactAssessment regulation_refs 引用 |
| **R264** | 验证会签 UI 已存在 |
| **R265** | 状态枚举替换（4 处硬编码 → RequirementStatus.BASELINE）|
| **R266** | 版本历史机制（编辑时自动写 t_requirement_version）|
| **R267** | 评审拒绝路径验证（CR-102-0004 → CLOSED）|

### 累计成果

- **PRJ-000004 R258-E2E-Test 项目**：4 条需求 + 1 个 LOCKED 基线 + 4 个变更请求（其中 2 个走完完整流程）
- **bug 修复**：ProjectService.create SY0000 / 变更类型枚举 / 基线状态联动 / performImpactAssessment / 版本历史
- **R255 决策彻底执行**：电子签名 UI 全部移除

### 关键技术决策

1. **R255 电子签名下线决策**：严格执行"一概不考虑"原文，UI 完全移除（不仅是 feature flag 拦截）
2. **变更管理边界**：CR 必基于 Baseline 需求，未基线化需求 R255 边界硬性拒绝
3. **基线系统设计**：锁定基线时联动更新需求 status='Baseline'（R261）
4. **跨模块查询**：BaselineService 已直接用 RequirementMapper（既有约定，不引入 JdbcTemplate 改造）

### 经验教训

1. **Chrome DevTools MCP 测试比纯 API 更真实**：R256.1 catch 静默吞错、R260 签名按钮残留等 bug 都是 UI 层发现的
2. **边界约束三层防护**：前端 UI 友好引导 + 后端硬性校验 + 系统设计联动，缺一不可
3. **SQL 绕过有时是测试唯一手段**：R262 中 performImpactAssessment bug 用 SQL 直插 t_impact_assessment
4. **枚举 vs 硬编码**：R265 替换后语义更严谨，但需保留测试字符串断言（不破坏历史用例）
5. **R 节点规范的价值**：CLAUDE.md §8.3 改前先建节点 → 改中编辑 → 改后 commit + tag，13 个节点无一次遗漏

### 待办（后续 R 节点候选）

- 追溯管理模块端到端测试
- 风险管理模块端到端测试  
- 合规管理（IEC 62304 / DHF / eRPS）端到端测试
- 报表与审计模块端到端测试
